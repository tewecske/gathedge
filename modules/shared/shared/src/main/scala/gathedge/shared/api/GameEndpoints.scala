...
import gathedge.shared.domain.{GameRef, Tag}
...
object GameEndpoints {
  ...
  /** The games whose wordlist set is '''exactly''' `tagIds` — not the games that merely carry one of them, which is
    ...
    *
    * Anonymous-capable, the same reasoning [[get]] applies: the setup screen is reachable signed out. An empty or
  ...
  /** [[get]], [[playSetup]], [[allGames]] and [[sameTagGames]] — a shared game link, the play-variant picker's preview
    * it leads to, the catalog of every account's games, and the read that says whether a wordlist set already has a
    * game — must all be viewable before any guest is minted, the same reasoning [[WordEndpoints.public]] applies to the
    * dictionary reads.
    */
  val public: List[Endpoint[?, ?, ?, ?, ?]] = List(get, playSetup, allGames, sameTagGames)
}
