...
object App {
  ...
  private def renderPage(page: Page, user: Option[User]): HtmlElement = {
    (page, user) match {
      ...
      case (Page.SignUp, _) =>
        SignUpPage.render()
      case (Page.Home, Some(u)) =>
        TodoPage.render(u)
      case (Page.Groups, Some(u)) =>
        GroupsPage.render(u)
      case (Page.GroupDetail(id), Some(u)) =>
        GroupDetailPage.render(u, id)
      case (Page.GroupMembers(id), Some(u)) =>
        GroupMembersPage.render(u, id)
      case (Page.AcceptInvite(token), _) =>
        AcceptInvitePage.render(user, token)
      case (Page.Admin, Some(u)) if u.isAdmin =>
        AdminUsersPage.render(u)
      case (Page.Admin, Some(_)) =>
        ForbiddenPage.render()
      case (Page.AdminUserDetail(id), Some(u)) if u.isAdmin =>
        AdminUserDetailPage.render(u, id)
      case (Page.AdminUserDetail(_), Some(_)) =>
        ForbiddenPage.render()
      case (Page.Forbidden, _) =>
        ForbiddenPage.render()
      case (Page.NotFound, _) =>
        NotFoundPage.render()
      case _ =>
        NotFoundPage.render()
    }
