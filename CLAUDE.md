...
**A word belongs to nobody; a tag is owned by exactly one account but visible to every account, and tagging is the whole of "this is in my vocabulary."** There's no `user_words` table — `mine=true` is a join through `word_tags`. Wiktionary rows and user-typed rows share the same table and search, told apart by `source`. `POST /api/words` is *ensure and attach*, not create-or-409 — two learners adding the same word is the normal case, and the second gets the first's translations.

**Ownership gates writes, not visibility.** `tags.user_id` still names the one account that may attach or detach a word (`WordService.requireOwnTag`, answering `TagNotFound` for anyone else's id — whose tag a given id is is not something an account may learn by trying), but `GET /api/tags` answers every tag in the system, marked `ownedByMe`, so a reader can filter or set their collect tag by anybody's. `POST /api/tags/{id}/copy` seeds a tag of the copier's own from another tag's name and copies that source tag's word memberships and marked pairs into it as a snapshot, in one repository transaction — the copy and the source are independent from that moment on, not a live link. Tag names are still unique per owner, case-insensitively, and always were — nothing about global visibility needed a schema change, since two accounts using the same name (`saved`, `lesson1`) was already allowed.

**Two standing per-account quotas bound the vocabulary** (`AppConfig.quotas`, `.env.example`): how many tags one account may own, and how many `word_tag_pairs` rows it may own summed across every tag it holds (a marked translation is two rows, one per direction). Not time-windowed like `RateLimiter` — these are a persistent count against what the account currently holds, checked and enforced entirely in `WordService` (`checkQuota`/`tagQuota`/`pairQuota`), never touching `RateLimitKey`. Each has a *soft* threshold, which lets the write through with a warning (`dto.TagResponse.warning`/`dto.PairSelectionResponse.warning`, a `MessageRef` the caller renders), and a *hard* one, which refuses it with `WordFailure.TagQuotaExceeded`/`PairQuotaExceeded` (409). An account may hold up to and including the hard limit; the write that would exceed it is the one refused. Enforced at `createTag` (including the tag `copyTag` creates), at `selectPair` (never charged for a pair already marked — nothing new is written), and at `copyTag` itself, which checks both dimensions against what the snapshot would add *before* writing anything, so a copy blocked on either hard limit leaves no partial tag or pairs behind.
