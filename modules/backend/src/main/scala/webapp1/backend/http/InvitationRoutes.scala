...
object InvitationRoutes {
  ...
  private def groupFailureResponse(failure: GroupFailure): Response = {
    failure match {
      ...
      case _ =>
        errorResponse(Status.BadRequest, "Could not process this invitation")
    }
  }

  private val getInvitationRoute = {
    Method.GET / "api" / "invitations" / string("token") ->
      handler { (token: String, request: Request) =>
        (
          for {
  ...
  val routes: Routes[AuthService & GroupService, Nothing] = Routes(getInvitationRoute, acceptInvitationRoute)
}
