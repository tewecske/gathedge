...
- **The HTTP server span** is ours: `RouteSupport.serverSpan`, a `HandlerAspect` built like `requestLogging`, attached last in `Main.allRoutes` so it is the parent of the log line, the handler, and the SQL spans below. The `usage_events` write is not under it: it runs on `UsageTracker`'s drain fiber in its own trace, opened as a `CONSUMER` span with a **span link** back to the server span (`record` captures the `SpanContext`, the drain loop passes it as `links`). `telemetry.Telemetry` is the only wiring (`OpenTelemetry.global` + `contextJVM` + `tracing`).
...
- **`gender` is part of a word's identity** (`der See` and `die See` are two rows). `NOT NULL` with `''` for "not gendered". The column stores the gender itself (`masculine`/`feminine`/`neuter`), not an article. **A declined form carries it too**, inherited from the word it inflects — by `V28` for the rows already stored, by the wiktextract import, by `WordService.create` when the caller names a main word, and by the tabular import's extra columns. Three shapes are left at `''` rather than guessed: a form whose own tags state a different gender (`Wörtlein` is `diminutive,neuter`), one whose lemmas disagree (`Lieben` inflects both `Lieber` and `Liebe` — always through a plural, where German's article ignores gender), and one whose gendered identity is already another row's.
...
**`LanguageProfile` (`shared/domain/LanguageProfile.scala`) is the only place an article literal may appear.** It maps each `WordLanguage` to the genders it has, **a whole declension table** (`declinedArticles`: one article per `FormSlot` × gender), the article forms its parser recognises, and whether its nouns capitalize. Every display, strip, or picker goes through it — a fifth language is a profile entry, not a grep for `WordLanguage.De`.

**The article comes from a cell, not from a gender.** `FormSlot` (`shared/domain/GrammaticalForm.scala`) is a case plus a number; `GrammarTag.slotOf` reads one out of a `word_forms.relation` string, leniently — no case reads as nominative, no number as singular, and several cases are resolved in `GrammaticalCase.all`'s order. A lemma stands in `FormSlot.citation`, so `der Tisch` and `den Tischen` come out of one lookup rather than two rules, and `definiteArticles` is now *derived* from that cell rather than stored beside it. Gender alone was never enough: `die Sache` pluralises to `die Sachen` in the nominative and `den Sachen` in the dative.
...
**A play picks how much of the article system it wants** (`ArticleMode`, on `game_plays.article_mode`, replacing the `include_definite_articles` boolean V29 dropped): `All` offers every article the language has in the play page's picker, `FormSpecific` only the ones the asked cell allows — three in the nominative singular, two in the dative singular, one in any plural — and `Off` shows none. **The two article modes accept the same answer**; they differ only in how many buttons the picker draws. `GamePrompt.answerSlot` is what the picker narrows by and what names the case beside the prompt, and it describes the *answer*, never the shown word.

**A form is asked in exactly one cell, and the choice is never a draw.** `FormSlot.preferred` picks it: the nominative wherever the form has one, and otherwise the form's own cell, so `Nächte` is `die Nächte` and the genitive-singular-only `Tisches` is `des Tisches`. **The case dominates the number in that ordering** — `Wege` carries the archaic dative singular `dem Wege` beside its three plural cells, and a number-first rule would ask a plural word in the singular.

Not a draw, because a `word_tag_pairs` row records *one* reading: somebody who marked `éjszakák` against `Nächte` meant the plural, and nothing in that pair justifies asking the genitive plural `der Nächte` instead. `startPlay` still freezes the chosen relation onto `game_play_words` (`word_relation`/`translation_relation`, `NULL` for a word that inflects nothing) for the reason the deleted `game_word_pool` gave for its own draw: a form edge imported mid-play must not change a question already asked. Words a play never sampled — the other accepted translations of an answered word, a multiple-choice distractor — go through the same `FormSlot.preferred` via `GameService.slotsFor`, so the two can never disagree.

**Both the prompt and the results row name the cell** (`GamePrompt.answerSlot`, `GameAnswerResult.answerSlot`), and only when it is not the citation one — an article like `des Tisches` is otherwise unaccountable from the screen alone.

**One game per wordlist set is a recommendation, not a rule.** `Tag.soloGame` is the game built from that wordlist *alone* — the oldest one where there are several, so every reader is sent to the same game. It is what makes the wordlist catalog and the wordlist editor draw "Play game" instead of "Create game", and it is filled by the two reads those pages already make (`GET /api/tags/page`, `GET /api/tags/{tagId}`) rather than by a request of its own; every other read leaves it `None`. `GET /api/games/same-tags` answers the games whose wordlist set is *exactly* the given one — the setup screen's warning, and not `allGames`'s `tag` filter, which answers "carries this wordlist". Neither refuses anything: a multi-wordlist game is ordinary, and `GET /api/admin/games/same-tags` is where an administrator reviews the sets that grew a second game anyway.
...
- Rows are written **sequentially**, since `tagMemberships` answers in insertion order and that is the reader's own row order. `importPair` marks them `PairMatch.Paired`, not `Verified`: the file asserts the pair, and nothing has checked it. Neither bulk path is pair-quota-gated; `maxTabularRows` and the shared `RateLimitKey.wordUpload` budget are the bounds.
- **`word_tag_pairs.match_kind` says who made a pair, and the two import paths never share a badge.** `''` is a hand-marked pair, `'verified'` a free-text import that found the two words already linked in `word_translations`, `'paired'` a tabular row that put them on one line. `PairMatch` (in `shared/domain/Tag.scala`) is the enum; the editor's provenance filters and badges read it, and a hand edit through `replacePair` writes a pair back to `Manual`.
