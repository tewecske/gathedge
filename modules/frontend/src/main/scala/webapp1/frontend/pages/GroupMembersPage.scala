...
private class GroupMembersPage(groupId: Long) {
  ...
  def render(): HtmlElement = {
    div(
      ...
      child.maybe <--
        groupVar.signal.map(_.map(g => GroupSubmenu.render(groupId, Page.GroupMembers(groupId), g.myRole))),
      ...
      loadBus.events.flatMapSwitch(_ => ApiClient.get[List[GroupMember]](s"/api/groups/$groupId/members")) -->
          ...
          table(
            ...
            tbody(
              children <--
                membersVar
                  .signal
                  .splitSeq(_.userId) { memberSignal =>
                    renderMemberRow(memberSignal)
                  }
            ),
          ),
        ),
  ...
  private def invite() = {
    ...
    if (email.trim.isEmpty) {
      ...
      ApiClient
        .postBodyNoContent(s"/api/groups/$groupId/invitations", InviteMemberRequest(email, role))
        .map {
          case Right(_) =>
            inviteEmailVar.set("")
            errorVar.set(None)
