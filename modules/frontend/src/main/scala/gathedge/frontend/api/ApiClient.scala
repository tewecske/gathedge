...
object ApiClient {
  ...
  /** Where the browser must be *navigated* to start a social sign-in — deliberately a URL rather than a call.
    ...
    * the provider and back, so it has to be the document that navigates, not an XHR. That also puts them outside
    * [[HttpClient]] entirely, which is why this is the one place in the frontend that spells out an API path for a
