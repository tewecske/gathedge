...
enum AdminFailure {
  ...
  case DuplicateEmail
  case NotFound
  /** An administrator can't remove their own admin privileges or delete their own
    * account — enforced here independent of the UI (summary.md).
    */
  case SelfDemote
  case SelfDelete
}
...
trait AdminService {
  ...
  def updateUser(actingAdminId: Long, id: Long, email: String, isAdmin: Boolean, password: Option[String]): IO[AdminFailure, User]
  def deleteUser(actingAdminId: Long, id: Long): IO[AdminFailure, Unit]
}

/** Admin actions that change accounts (create/promote-or-demote/delete) are logged
  * to the `security` logger as an audit trail, per summary.md's "logging for
  * application events, such as user actions".
  */
final class AdminServiceLive(userRepo: UserRepository, hasher: PasswordHasher) extends AdminService {
  ...
  def updateUser(
    ...
    for {
      ...
      _ <- ZIO.fromEither(Validation.validateEmail(normalizedEmail)).mapError(err => AdminFailure.ValidationError(Map("email" -> err)))
      ...
      _ <- ZIO.when(before.isAdmin != isAdmin) {
...
object AdminServiceLive {
  val live: URLayer[UserRepository & PasswordHasher, AdminService] =
    ZLayer.fromFunction((u: UserRepository, h: PasswordHasher) => new AdminServiceLive(u, h): AdminService)
}
