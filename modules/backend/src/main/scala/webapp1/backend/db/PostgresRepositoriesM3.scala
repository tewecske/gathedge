...
final class PostgresGroupInvitationRepository(dataSource: DataSource) extends GroupInvitationRepository {
...
object PostgresGroupInvitationRepository {
  val live: ZLayer[DataSource, Nothing, GroupInvitationRepository] =
    ZLayer.fromFunction((ds: DataSource) => new PostgresGroupInvitationRepository(ds): GroupInvitationRepository)
}
