...
object AdminSubmenu {
  def render(active: Page): HtmlElement = {
    div(
      ...
      tabLink(Page.Admin, "Users", isUsersTab(active)),
      tabLink(Page.AdminAudit, "Audit log", active == Page.AdminAudit),
      tabLink(Page.AdminSystem, "System", active == Page.AdminSystem),
    )
  }
