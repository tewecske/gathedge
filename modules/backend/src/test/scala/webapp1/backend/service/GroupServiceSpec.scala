package webapp1.backend.service

import webapp1.backend.{TestAuthLayers, TestDataSource}
import webapp1.backend.config.AppConfig
import webapp1.backend.db.{
  GroupInvitationRepository,
  GroupInvitationRow,
  GroupMemberRepository,
  GroupPairRepository,
  GroupRepository,
  UserRepository,
}
import webapp1.shared.i18n.{MessageKeys, MessageRef}
import webapp1.shared.domain.GroupRole
import zio._
import zio.test._

import java.util.concurrent.TimeUnit

object GroupServiceSpec extends ZIOSpecDefault {

  private val repoLayer = {
    TestDataSource.sqlite >>> (
      GroupRepository.test ++ GroupMemberRepository.test ++ GroupPairRepository.test ++
        GroupInvitationRepository.test ++ UserRepository.test
    )
  }

  // repoLayer is referenced twice below; ZIO memoizes same-instance layers within
  // one composition, so both branches share a single underlying (SQLite) DB.
  private val layer: ZLayer[Any, Throwable, GroupService & GroupInvitationRepository & UserRepository] =
    repoLayer ++ ((repoLayer ++ TestAuthLayers.emailAndConfig) >>> GroupService.live)

  def spec = suite("GroupService (SQLite)")(
    test("creating a group makes the creator an admin member") {
      for {
        group <- GroupService.createGroup(1L, "Acme")
      } yield assertTrue(group.name == "Acme", group.myRole == GroupRole.Admin)
    },
    test("rejects a blank group name") {
      for {
        result <- GroupService.createGroup(1L, "  ").either
      } yield assertTrue(result.isLeft)
    },
    test("a non-member cannot list or add pairs (private to the owning group)") {
      for {
        group      <- GroupService.createGroup(1L, "Private Co")
        listResult <- GroupService.listPairs(2L, group.id).either
        addResult  <- GroupService.addPair(2L, "intruder@example.com", group.id, "a", "b").either
      } yield assertTrue(listResult == Left(GroupFailure.NotMember), addResult == Left(GroupFailure.NotMember))
    },
    test("member can add a pair; blank source/target is rejected") {
      for {
        group       <- GroupService.createGroup(1L, "Writable Co")
        pair        <- GroupService.addPair(1L, "owner@example.com", group.id, "src", "tgt")
        pairs       <- GroupService.listPairs(1L, group.id)
        blankResult <- GroupService.addPair(1L, "owner@example.com", group.id, "", "tgt").either
      } yield assertTrue(
        pair.source == "src",
        pair.target == "tgt",
        pair.createdByEmail == "owner@example.com",
        pairs.map(_.id) == List(pair.id),
        blankResult ==
          Left(
            GroupFailure.ValidationError(
              Map("source" -> MessageRef(MessageKeys.fieldRequired, List(MessageRef.keyArg(MessageKeys.fieldSource))))
            )
          ),
      )
    },
    test("accessing a nonexistent group fails with NotFound") {
      for {
        result <- GroupService.listPairs(1L, 999999L).either
      } yield assertTrue(result == Left(GroupFailure.NotFound))
    },
    test("only a group admin can invite, remove members, change roles, or delete the group") {
      for {
        group        <- GroupService.createGroup(1L, "Admin-only Co")
        now          <- Clock.currentTime(TimeUnit.MILLISECONDS)
        invite       <- GroupInvitationRepository.insert(
                          GroupInvitationRow(
                            0L,
                            group.id,
                            "member@example.com",
                            "read_write",
                            "tok-nonadmin",
                            1L,
                            now,
                            now + 100000L,
                            None,
                          )
                        )
        _            <- GroupService.acceptInvitation(2L, "member@example.com", invite.token)
        inviteResult <- GroupService.inviteMember(2L, group.id, "third@example.com", GroupRole.ReadOnly).either
        removeResult <- GroupService.removeMember(2L, group.id, 1L).either
        roleResult   <- GroupService.updateMemberRole(2L, group.id, 1L, GroupRole.ReadOnly).either
        deleteResult <- GroupService.deleteGroup(2L, group.id).either
      } yield assertTrue(
        inviteResult == Left(GroupFailure.AdminOnly),
        removeResult == Left(GroupFailure.AdminOnly),
        roleResult == Left(GroupFailure.AdminOnly),
        deleteResult == Left(GroupFailure.AdminOnly),
      )
    },
    test("the sole admin cannot be removed or demoted (last-admin protection)") {
      for {
        group        <- GroupService.createGroup(1L, "Solo Admin Co")
        removeResult <- GroupService.removeMember(1L, group.id, 1L).either
        demoteResult <- GroupService.updateMemberRole(1L, group.id, 1L, GroupRole.ReadWrite).either
      } yield assertTrue(removeResult == Left(GroupFailure.LastAdmin), demoteResult == Left(GroupFailure.LastAdmin))
    },
    test("promoting a second admin allows the first admin to then leave") {
      // listMembers joins group_members with real user rows, so this test (unlike
      // the others) needs actual users rather than arbitrary literal ids.
      for {
        now0         <- Clock.currentTime(TimeUnit.MILLISECONDS)
        admin1       <-
          UserRepository.insert("admin1@twoadmins.example.com", None, isAdmin = false, "light", "en", now0, Some(now0))
        admin2       <-
          UserRepository.insert("admin2@twoadmins.example.com", None, isAdmin = false, "light", "en", now0, Some(now0))
        group        <- GroupService.createGroup(admin1.id, "Two Admins Co")
        now          <- Clock.currentTime(TimeUnit.MILLISECONDS)
        invite       <- GroupInvitationRepository.insert(
                          GroupInvitationRow(
                            0L,
                            group.id,
                            admin2.email,
                            "read_write",
                            "tok-promote",
                            admin1.id,
                            now,
                            now + 100000L,
                            None,
                          )
                        )
        acceptResult <- GroupService.acceptInvitation(admin2.id, admin2.email, invite.token)
        _            <- GroupService.updateMemberRole(admin1.id, group.id, admin2.id, GroupRole.Admin)
        removeResult <- GroupService.removeMember(admin1.id, group.id, admin1.id)
        members      <- GroupService.listMembers(admin2.id, group.id)
      } yield assertTrue(
        acceptResult.myRole == GroupRole.ReadWrite,
        removeResult == (),
        members.map(_.userId) == List(admin2.id),
      )
    },
    test("accepting an invitation with a mismatched email is rejected") {
      for {
        group  <- GroupService.createGroup(1L, "Mismatch Co")
        now    <- Clock.currentTime(TimeUnit.MILLISECONDS)
        invite <- GroupInvitationRepository.insert(
                    GroupInvitationRow(
                      0L,
                      group.id,
                      "invited@example.com",
                      "read_only",
                      "tok-mismatch",
                      1L,
                      now,
                      now + 100000L,
                      None,
                    )
                  )
        result <- GroupService.acceptInvitation(2L, "someone-else@example.com", invite.token).either
      } yield assertTrue(result == Left(GroupFailure.InvitationInvalid))
    },
    test("accepting an expired invitation is rejected") {
      for {
        group  <- GroupService.createGroup(1L, "Expired Co")
        now    <- Clock.currentTime(TimeUnit.MILLISECONDS)
        invite <- GroupInvitationRepository.insert(
                    GroupInvitationRow(
                      0L,
                      group.id,
                      "late@example.com",
                      "read_only",
                      "tok-expired",
                      1L,
                      now - 200000L,
                      now - 100000L,
                      None,
                    )
                  )
        result <- GroupService.acceptInvitation(2L, "late@example.com", invite.token).either
      } yield assertTrue(result == Left(GroupFailure.InvitationInvalid))
    },
  ).provide(layer)
}
