...
object AdminUserDetailPage {
  def render(currentUser: User, userId: Long): HtmlElement =
    AppShell.render(currentUser, Page.AdminUserDetail(userId), new AdminUserDetailPage(userId).render())
}
...
private class AdminUserDetailPage(userId: Long) {
  ...
  def render(): HtmlElement = {
    div(
      ...
      loadBus.events.flatMapSwitch(_ => ApiClient.get[User](s"/api/admin/users/$userId")) --> Observer[Either[ApiError, User]] {
        case Right(u) =>
          userVar.set(Some(u))
          emailVar.set(u.email)
          isAdminVar.set(u.isAdmin)
          notFoundVar.set(false)
        case Left(err) if err.status == 404 => notFoundVar.set(true)
    ...
    div(
      ...
      div(
        ...
        fieldSet(
          ...
          input(cls := "input w-full", typ := "email", value <-- emailVar.signal, onInput.mapToValue --> emailVar.writer),
        ...
        div(
          ...
          button(
            ...
            onClick.mapToUnit --> Observer[Unit] { _ =>
              if (dom.window.confirm("Delete this user? This cannot be undone.")) deleteBus.emit(())
            },
          ),
  ...
  private def save() = {
    ...
    ApiClient.put[UpdateUserRequest, User](s"/api/admin/users/$userId", UpdateUserRequest(emailVar.now(), isAdminVar.now(), password)).map {
      case Right(u) =>
        userVar.set(Some(u))
        passwordVar.set("")
        errorVar.set(None)
  ...
  private def delete() = {
    ApiClient.delete(s"/api/admin/users/$userId").map {
      case Right(_)  => AppRouter.router.pushState(Page.Admin)
      case Left(err) => errorVar.set(Some(err.message))
    }
  }
