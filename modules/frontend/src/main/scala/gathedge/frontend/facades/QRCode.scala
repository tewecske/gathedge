...
/** Thin facade over the `qrcode` npm package (pinned in `web/package.json`) — the only client-side dependency this app
  ...
  *
  * `Namespace` rather than `Default`: the package is plain CommonJS (`exports.toDataURL = …`, no `__esModule` flag), and
  * a namespace import is what survives both of Vite's two bundling paths (esbuild's CJS shim in dev, Rollup's
...
object QRCode extends js.Object {
  ...
  /** Resolves to a `data:image/png;base64,…` URI encoding `text` as a QR code. Rejects only if `text` cannot be
    * encoded at all (never for a network reason — there is no network call).
    */
