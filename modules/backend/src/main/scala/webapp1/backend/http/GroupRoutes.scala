...
object GroupRoutes {
  ...
  private def groupFailureResponse(failure: GroupFailure): Response = failure match {
    case GroupFailure.ValidationError(fieldErrors) => errorResponse(Status.BadRequest, "Validation failed", fieldErrors)
    case GroupFailure.NotFound                     => errorResponse(Status.NotFound, "Group not found")
    case GroupFailure.NotMember                    => errorResponse(Status.Forbidden, "You are not a member of this group")
    case GroupFailure.ReadOnlyMember                => errorResponse(Status.Forbidden, "Your role in this group is read-only")
    case GroupFailure.AdminOnly                     => errorResponse(Status.Forbidden, "Only a group administrator can do this")
    case GroupFailure.LastAdmin =>
      errorResponse(Status.Conflict, "A group must always have at least one administrator; promote another member first")
  ...
  private val createGroupRoute =
    Method.POST / "api" / "groups" -> handler { (request: Request) =>
      (for {
        _            <- csrfCheck(request)
      ...
      } yield jsonResponse(Status.Created, group)).merge
    }

  private val listGroupsRoute =
    Method.GET / "api" / "groups" -> handler { (request: Request) =>
      (for {
        user         <- authenticatedUser(request)
        groupService <- ZIO.service[GroupService]
        groups       <- groupService.myGroups(user.id)
      } yield jsonResponse(Status.Ok, groups)).merge
    }

  private val getGroupRoute =
    Method.GET / "api" / "groups" / long("id") -> handler { (groupId: Long, request: Request) =>
      (for {
        user         <- authenticatedUser(request)
        groupService <- ZIO.service[GroupService]
  ...
  private val deleteGroupRoute =
    Method.DELETE / "api" / "groups" / long("id") -> handler { (groupId: Long, request: Request) =>
      (for {
        _            <- csrfCheck(request)
        user         <- authenticatedUser(request)
        groupService <- ZIO.service[GroupService]
  ...
  private val listPairsRoute =
    Method.GET / "api" / "groups" / long("id") / "pairs" -> handler { (groupId: Long, request: Request) =>
      (for {
        user         <- authenticatedUser(request)
        groupService <- ZIO.service[GroupService]
        pairs        <- groupService.listPairs(user.id, groupId).mapError(groupFailureResponse)
      } yield jsonResponse(Status.Ok, pairs)).merge
    }

  private val addPairRoute =
    Method.POST / "api" / "groups" / long("id") / "pairs" -> handler { (groupId: Long, request: Request) =>
      (for {
        _            <- csrfCheck(request)
        user         <- authenticatedUser(request)
      ...
      } yield jsonResponse(Status.Created, pair)).merge
    }

  private val listMembersRoute =
    Method.GET / "api" / "groups" / long("id") / "members" -> handler { (groupId: Long, request: Request) =>
      (for {
        user         <- authenticatedUser(request)
        groupService <- ZIO.service[GroupService]
        members      <- groupService.listMembers(user.id, groupId).mapError(groupFailureResponse)
      } yield jsonResponse(Status.Ok, members)).merge
    }

  private val removeMemberRoute =
    Method.DELETE / "api" / "groups" / long("id") / "members" / long("userId") -> handler {
      (groupId: Long, targetUserId: Long, request: Request) =>
        (for {
          _            <- csrfCheck(request)
          user         <- authenticatedUser(request)
          groupService <- ZIO.service[GroupService]
  ...
  private val updateRoleRoute =
    Method.PUT / "api" / "groups" / long("id") / "members" / long("userId") -> handler {
      (groupId: Long, targetUserId: Long, request: Request) =>
        (for {
          _            <- csrfCheck(request)
          user         <- authenticatedUser(request)
  ...
  private val inviteMemberRoute =
    Method.POST / "api" / "groups" / long("id") / "invitations" -> handler { (groupId: Long, request: Request) =>
      (for {
        _            <- csrfCheck(request)
        user         <- authenticatedUser(request)
