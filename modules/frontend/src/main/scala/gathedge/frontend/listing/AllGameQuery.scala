...
/** Everything that decides which games the server sends back — the games-listing counterpart of [[MyPlayQuery]], for
  * `GET /api/games/all`. Same four paging questions plus a `search`: here it is a substring of the game's own name. The
  * sort vocabulary is its own ([[AllGameSort]]): only the name and the creation date order, since the play count is an
  * aggregate this listing does not join and the rest are labels.
  */
...
object AllGameQuery {
  ...
  /** The query half of `/games/all`. `q` is the name filter: a substring of the game's name, matched case-insensitively
    * by the server.
    */
  val params = (ListingParams.common & param[String]("q").?).as[AllGameQuery](using codec)
}
