...
final case class SystemServiceLive(
  ...
  def prune(actor: AdminActor): UIO[PruneResult] = {
    for {
      ...
      swept             <- SessionReaper.sweep.provideEnvironment(ZEnvironment(sessionRepo, tokenRepo)).orDie
      (sessions, tokens) = swept
      ...
      result             = PruneResult(sessions, tokens, keys)
      _                 <- auditTrail.recordSystem(
                             actor,
                             AuditAction.systemPrune,
                             s"pruned $sessions session(s), $tokens verification token(s) and $keys rate-limit key(s)",
                           )
    } yield result
  }
}
