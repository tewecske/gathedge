...
/** The "Bulk upload" modal on the Words page: pick a text file, review what the dictionary already knows about it, and
  ...
  * is the call carrying the file's text, and `HttpClient`'s `fetch`-based calls expose no upload-progress event, so
  * preview speaks to its endpoint directly with a plain `XMLHttpRequest`, sending and decoding the same DTOs
  * `HttpClient` would. Confirm's body is small (ids and a handful of short strings), so it is an ordinary
  * `WordApiClient` call.
  *
