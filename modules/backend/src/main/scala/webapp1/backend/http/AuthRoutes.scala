...
object AuthRoutes {
  ...
  private def authFailureResponse(failure: AuthFailure): Response = failure match {
  ...
  private val signupRoute =
    Method.POST / "api" / "auth" / "signup" -> handler { (request: Request) =>
      (for {
        _           <- csrfCheck(request)
  ...
  private val loginRoute =
    Method.POST / "api" / "auth" / "login" -> handler { (request: Request) =>
      (for {
        ...
        val (user, sessionId) = result
        jsonResponse(Status.Ok, AuthResponse(user))
          .addCookie(SessionAuth.buildSessionCookie(sessionId, cfg.session.cookieSecure))
      }).merge
    }

  private val logoutRoute =
    Method.POST / "api" / "auth" / "logout" -> handler { (request: Request) =>
      (for {
        _           <- csrfCheck(request)
        authService <- ZIO.service[AuthService]
        cfg         <- ZIO.service[AppConfig]
        // Logging out with no session cookie is a no-op, not an error.
        _ <- SessionAuth.sessionIdFrom(request) match {
               case Some(sid) => authService.logout(sid)
               case None      => ZIO.unit
             }
      } yield {
  ...
  private val updateThemeRoute =
    Method.PUT / "api" / "me" / "theme" -> handler { (request: Request) =>
      ...
      } yield jsonResponse(Status.Ok, AuthResponse(updated))).merge
    }

  private val googleStartRoute =
    Method.GET / "api" / "auth" / "google" / "start" -> handler { (request: Request) =>
      (for {
  ...
  private val googleCallbackRoute =
    Method.GET / "api" / "auth" / "google" / "callback" -> handler { (request: Request) =>
      (for {
        code  <- ZIO.fromOption(request.queryParam("code")).orElseFail(errorResponse(Status.BadRequest, "Missing code"))
        state <- ZIO.fromOption(request.queryParam("state")).orElseFail(errorResponse(Status.BadRequest, "Missing state"))
        cookieState <- ZIO
  ...
  val routes: Routes[AuthService & GoogleOAuthClient & AppConfig, Nothing] =
    Routes(
      signupRoute,
      loginRoute,
      logoutRoute,
