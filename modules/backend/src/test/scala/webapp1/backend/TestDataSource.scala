...
/** A fresh, migrated SQLite DB per layer instantiation — the test side of the
  * dual-dialect DB strategy. Shared by every *ServiceSpec so each test gets full
  * isolation without duplicating the setup boilerplate.
  */
object TestDataSource {
  val sqlite: ZLayer[Any, Throwable, DataSource] =
    ZLayer.scoped {
      for {
