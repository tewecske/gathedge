...
- **Each endpoint declares exactly the statuses it can answer with, and its error channel is the union of those.** Not one uniform set: `ApiEndpoint.failure` is the codec vocabulary and `.failingWith(...)` attaches a subset, e.g. `GET /api/me` declares only 401 and 500 while `PUT /api/groups/{id}/members/{userId}` declares six. The list is its handler's failures (whatever the `ApiFailures` mapper produces — nothing at all when the service call is a `UIO`) **plus what the aspects can answer instead of running it**: 401 under `authenticated`/`adminOnly`, 403 under `adminOnly` or under `csrf` for a non-GET, 500 everywhere via `handleFailures`. That second group is mandatory — those responses bypass the endpoint's codecs but a client built from the description still has to decode them, and a status a description omits is not decodable at all: the endpoint client fails it as a *defect* reading "Expected status code ... but found Unauthorized", so an expired session would reach the page as an unrenderable crash instead of a 401.
  - `.failingWith` exists rather than calling `outErrors` directly because it **infers the union from the codecs passed in**. `outErrors[E]` takes `E` explicitly and only checks each codec is a subtype, so a union wider than its codec list compiles and then fails at encode time; inferring it makes the two impossible to disagree. In the other direction, a handler that fails with `ApiFailure.Conflict` on an endpoint that doesn't declare 409 no longer compiles.
  - `OpenApiSpec` pins the whole per-operation status table (125 declared failure statuses across 25 operations, down from a uniform 175), and `ApiEndpointsSpec` pins that the aspect-built bodies decode with the endpoints' own zio-schema codecs.
  - Residual slack: the ten group endpoints all declare the same six because every `GroupService` method is typed `IO[GroupFailure, ?]` and the mapper covers 400/403/404/409, even though e.g. `getGroup` cannot raise a 409. Tightening that means narrowing the service signatures, not the descriptions.
