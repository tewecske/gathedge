...
/** A failed call, as the pages see it: the status, a message to show, and per-field messages for a form.
  ...
  * `shared.api.ApiFailure`), and `EndpointClient` resolves them at the seam — so a page renders `err.message` exactly
  * as it always did and needs to know nothing about i18n.
  *
