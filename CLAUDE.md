...
Every route file still exposes a `Routes[R, Response]` — an error channel of `Response` is what zio-http calls "handled". `Main.scala` concatenates all route files with `++`, then wraps the result in `RouteSupport.handleFailures`, which passes failure `Response`s through and turns defects (every `.orDie` in the services) into a logged cause plus a generic JSON 500. It also replaces `Routes.notFound` (a mutable field on `Routes`, and the only hook for it): a request matching no route never reaches a handler, so zio-http would otherwise answer it with `Response.error(NotFound, path)` — the requested path echoed back, no JSON, no content type — the one response a client could get that isn't the `ErrorResponse` shape.
