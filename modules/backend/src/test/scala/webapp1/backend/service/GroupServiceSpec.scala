...
object GroupServiceSpec extends ZIOSpecDefault {
  ...
  private val repoLayer =
    TestDataSource.sqlite >>> (SqliteGroupRepository.live ++ SqliteGroupMemberRepository.live ++ SqliteGroupPairRepository.live ++ SqliteGroupInvitationRepository.live ++ SqliteUserRepository.live)
  ...
  def spec = suite("GroupService (SQLite)")(
    ...
    test("only a group admin can invite, remove members, change roles, or delete the group") {
      for {
        ...
        invite <- invitationRepo.insert(
                    GroupInvitationRow(0L, group.id, "member@example.com", "read_write", "tok-nonadmin", 1L, now, now + 100000L, None)
                  )
    ...
    test("promoting a second admin allows the first admin to then leave") {
      ...
      for {
        ...
        now            <- Clock.currentTime(TimeUnit.MILLISECONDS)
        invite <- invitationRepo.insert(
                    GroupInvitationRow(0L, group.id, admin2.email, "read_write", "tok-promote", admin1.id, now, now + 100000L, None)
                  )
    ...
    test("accepting an invitation with a mismatched email is rejected") {
      for {
        ...
        now             <- Clock.currentTime(TimeUnit.MILLISECONDS)
        invite <- invitationRepo.insert(
                    GroupInvitationRow(0L, group.id, "invited@example.com", "read_only", "tok-mismatch", 1L, now, now + 100000L, None)
                  )
    ...
    test("accepting an expired invitation is rejected") {
      for {
        ...
        now            <- Clock.currentTime(TimeUnit.MILLISECONDS)
        invite <- invitationRepo.insert(
                    GroupInvitationRow(0L, group.id, "late@example.com", "read_only", "tok-expired", 1L, now - 200000L, now - 100000L, None)
                  )
