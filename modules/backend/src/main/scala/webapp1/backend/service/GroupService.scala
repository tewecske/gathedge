...
/** Group creation makes the creator an admin member (summary.md); every other
  * group operation requires membership, writes to pairs additionally require a
  * non-read-only role, and membership/invitation management requires the admin
  * role. Removing or demoting a group's last admin is always blocked — a superset
  * of summary.md's "last admin can't remove themselves" that also prevents ever
  * leaving a group with zero admins via any other actor.
  */
trait GroupService {
  ...
  def addPair(userId: Long, userEmail: String, groupId: Long, source: String, target: String): IO[GroupFailure, GroupPair]
...
final class GroupServiceLive(
  ...
  def createGroup(userId: Long, name: String): IO[GroupFailure, Group] = {
    Validation.validateNonBlank(name, "Name") match {
      case Left(err) => ZIO.fail(GroupFailure.ValidationError(Map("name" -> err)))
  ...
  def myGroups(userId: Long): UIO[List[Group]] = {
    groupRepo.listForUser(userId).orDie.map(_.map { case (row, role) => toDomain(row, role) })
  }
  ...
  private def requireAdmin(userId: Long, groupId: Long): IO[GroupFailure, Unit] = {
    requireMembership(userId, groupId).flatMap(role => ZIO.unless(role == GroupRole.Admin)(ZIO.fail(GroupFailure.AdminOnly))).unit
  }

  private def guardLastAdmin(groupId: Long, targetCurrentRole: Option[GroupRole]): IO[GroupFailure, Unit] = {
    ZIO.when(targetCurrentRole.contains(GroupRole.Admin)) {
  ...
  def listMembers(userId: Long, groupId: Long): IO[GroupFailure, List[GroupMember]] = {
    for {
      ...
      GroupMember(row.userId, email, GroupRole.fromString(row.role).getOrElse(GroupRole.ReadOnly), row.joinedAt.toString)
    }
  }
  ...
  def inviteMember(userId: Long, groupId: Long, email: String, role: GroupRole): IO[GroupFailure, Unit] = {
    ...
    for {
      _     <- requireAdmin(userId, groupId)
      _     <- ZIO.fromEither(Validation.validateEmail(normalizedEmail)).mapError(err => GroupFailure.ValidationError(Map("email" -> err)))
      group <- findGroupOrDie(groupId)
      now   <- Clock.currentTime(TimeUnit.MILLISECONDS)
      token  = newToken()
      _ <- invitationRepo
      ...
      _ <- emailSender
             .send(normalizedEmail, s"You're invited to join ${group.name}", s"Join the group here: $link")
             .orDie
    } yield ()
  }
  ...
  def acceptInvitation(userId: Long, userEmail: String, token: String): IO[GroupFailure, Group] = {
    for {
      ...
      _ <- existingRole match {
             case Some(_) => memberRepo.updateRole(invite.groupId, userId, invite.role).orDie
             case None    => memberRepo.addMember(invite.groupId, userId, invite.role, now).orDie
           }
...
object GroupServiceLive {
  val live
    : URLayer[GroupRepository & GroupMemberRepository & GroupPairRepository & GroupInvitationRepository & EmailSender & AppConfig, GroupService] =
    ZLayer.fromFunction(
      (
        g: GroupRepository,
