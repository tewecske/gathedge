...
**A form is asked in one case per play, and that case is frozen.** `game_play_words` carries `word_relation`/`translation_relation`, drawn by `startPlay` from the word's own `word_forms` rows (deduplicated by the cell they name, so a word reaching one cell through two relations is not twice as likely). `NULL` is a word that inflects nothing, which is every lemma. Frozen for the reason the deleted `game_word_pool` gave for its own draw: re-picking per request would let a prompt show `des Lieben` and grade against `den Lieben`, and a form edge imported mid-play must not change the question. Words a play never sampled — the other accepted translations of an answered word, a multiple-choice distractor — get a cell derived deterministically instead (`GameService.slotsFor`).
